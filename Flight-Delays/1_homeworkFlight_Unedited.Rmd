...
totalTime = 1000000 #### Answer 0.2 ## Overwrite the totalTime object so that it has the correct elapsed time
```
...
totalNum  = nrow(df)
cancelNum =  df %>%
  group_by(....)#### Answer 1.1 ## Insert the correct group_by statement here
  summarise(n = n())
...
flightsNotCancelled = df %>%
  filter(....) %>% #### Answer 1.2  Insert the correct filter statement here
  na.omit
```
...
The total number of flights is... .  The total number of cancelled flights is... . The total number of flights
remaining after removing NAs is .... 
