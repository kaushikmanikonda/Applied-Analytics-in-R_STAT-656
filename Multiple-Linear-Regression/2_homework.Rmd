...
#### Answer 1.1.1.2
diff = summer$mean - winter$mean
diff

#The sample mean difference between winter and summer is 1.427925
...
bootstrapResults[b] = winter_boot$mean - summer_boot$mean #### Answer 1.2.3 compute the sample mean difference
}
end = proc.time()[3]

sorted_bootresults = sort(bootstrapResults)

bootstrapCI = c(sorted_bootresults[1], sorted_bootresults[10]) #### Answer 1.2.4. 
...
foreach (b = 1:nBootstrapDraws) %dopar% {
  require(dplyr)
  flightsNotCancelled_boot = flightsNotCancelled[sample(1:n,n,replace=TRUE), ]
...
trainIndex = createDataPartition(flightsNotCancelled$DEP_DELAY, p = .5, list = FALSE) %>% as.vector(.)

validSplit = createDataPartition(flightsNotCancelled$DEP_DELAY[-trainIndex], p = .5, list = FALSE) %>% as.vector(.)
...
flightsNotCancelled = flightsNotCancelled %>% 
  mutate(DEP_DELAY_TRUNC = ifelse(DEP_DELAY<0, 0, DEP_DELAY)) #### Answer 2.2.1
```
...
#Based on the output from previous chunk, the original feature is better since it has a lower squared error.
...
# After using the training set to train the model, we use the validation set to tune the model and validate the fit. 
